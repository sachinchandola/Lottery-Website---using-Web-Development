...
if (isset($_GET['uid'])) {
    $id = $_GET['uid'];
    if ($_GET['type'] == 'edit') {
        $getdata = mysqli_query($con, "SELECT * FROM `homestays` WHERE `sno`=$id");
        ...
        if (isset($_POST['add'])) {
            ...
            $sql = "UPDATE homestays SET `name`='$name', `description`='$description', `state`='$state', `district`='$district', `phoneNo`='$phoneNo', `email`='$email', `gmap`='$gmap', `whatsapp`='$whatsapp', `offer`='$offer'  WHERE `sno`=$id";
            ...
            if ($con->query($sql) === TRUE) {
                ...
                for ($i = 0; $i < $c; $i++) {
                    ...
                    if (!empty($namee) && !empty($temp)) {
                        ...
                        $sql = "INSERT INTO imagehomestay (images, id) value ('$namee', '$id')";
                        $result = mysqli_query($con, $sql);
                    }
                }

                echo "<script>window.location.href='listhomestay.php'</script>";
                exit();
            } else {
        ...
        mysqli_query($con, "DELETE FROM `homestays`  WHERE sno =$id");
        mysqli_query($con, "DELETE FROM `imagehomestay`  WHERE id =$id");
    ...
    if (isset($_POST['add'])) {
        ...
        $gmap = $_POST['gmap'];
        // insert pages
        $page_sql = "INSERT INTO homestays (`name`, `description`, `state`, `district`, `offer`, `whatsapp`, `phoneNo`, `email`, `gmap` )
        ...
        if ($con->query($page_sql) === TRUE) {
            ...
            for ($i = 0; $i < $c; $i++) {
                ...
                if (!empty($namee) && !empty($temp)) {
                    move_uploaded_file($temp, './img/' . $namee);
                    $sql = "INSERT INTO imagehomestay (images, id) value ('$namee', '$last_inserted_id')";
                    $result = mysqli_query($con, $sql);
                }
            }
...
if(isset($_POST['img_delete'])){
    ...
    $img_id = $_POST['img_id'];
    // delete image
    mysqli_query($con, "DELETE FROM `imagehomestay`  WHERE `sno` =$img_id");
    echo "<script>window.location.href='edithomestay.php?type=edit&uid=' + hotel_id</script>";
}
?>
                ...
                <div class="col-sm-12 col-xl-12">
                    <div class="bg-secondary rounded h-100 p-4">
                        <div style='display:flex; justify-content: space-between'>
                            <h5 class="mb-4">Homestay</h5>
                            <a class="btn btn-primary" style='height:35px;' href='listhomestay'>Homestay List</a>
                        </div>

                        <form method='post' enctype="multipart/form-data" id='form' onsubmit="return valid()">
                            <div class="row mb-6">
                                <label for="inputEmail3" class="col-sm-2 col-form-label">Homestay Name</label>
                                <div>
                            ...
                            <div class="row mb-6">
                                ...
                                ?>
                                    ...
                                    <div id='images1'>
                                        <?php
                                        $sql_image = "SELECT * from `imagehomestay` where `id`='{$res_data['sno']}'";
