...
if (isset($_GET['uid'])) {
    $id = $_GET['uid'];
    if ($_GET['type'] == 'edit') {
        $getdata = mysqli_query($con, "SELECT * FROM `resorts` WHERE `sno`=$id");
        ...
        if (isset($_POST['add'])) {
            ...
            $sql = "UPDATE resorts SET `name`='$name', `description`='$description', `state`='$state', `district`='$district', `phoneNo`='$phoneNo', `email`='$email', `gmap`='$gmap', `whatsapp`='$whatsapp', `offer`='$offer'  WHERE `sno`=$id";
            ...
            if ($con->query($sql) === TRUE) {
                ...
                for ($i = 0; $i < $c; $i++) {
                    ...
                    if (!empty($namee) && !empty($temp)) {
                        ...
                        $sql = "INSERT INTO imageresort (images, id) value ('$namee', '$id')";
                        $result = mysqli_query($con, $sql);
                    }
                }

                echo "<script>window.location.href='listresort.php'</script>";
                exit();
            } else {
        ...
        mysqli_query($con, "DELETE FROM `resortss`  WHERE sno =$id");
        mysqli_query($con, "DELETE FROM `imageresort`  WHERE id =$id");
    ...
    if (isset($_POST['add'])) {
        ...
        $gmap = $_POST['gmap'];
        // insert pages
        $page_sql = "INSERT INTO resorts (`name`, `description`, `state`, `district`, `offer`, `whatsapp`, `phoneNo`, `email`, `gmap` )
        ...
        if ($con->query($page_sql) === TRUE) {
            ...
            for ($i = 0; $i < $c; $i++) {
                ...
                if (!empty($namee) && !empty($temp)) {
                    move_uploaded_file($temp, './img/' . $namee);
                    $sql = "INSERT INTO imageresort (images, id) value ('$namee', '$last_inserted_id')";
                    $result = mysqli_query($con, $sql);
                }
            }
...
if(isset($_POST['img_delete'])){
    ...
    $img_id = $_POST['img_id'];
    // delete image
    mysqli_query($con, "DELETE FROM `imageresort`  WHERE `sno` =$img_id");
    echo "<script>window.location.href='editresort.php?type=edit&uid=' + hotel_id</script>";
}
?>
                        ...
                        <form method='post' enctype="multipart/form-data" id='form' onsubmit="return valid()">
                            ...
                            <div class="row mb-6">
                                ...
                                ?>
                                    ...
                                    <div id='images1'>
                                        <?php
                                        $sql_image = "SELECT * from `imageresort` where `id`='{$res_data['sno']}'";
